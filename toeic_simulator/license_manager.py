...
Trial period  : 3 days from first launch, persisted in toeic_trial.dat
...
TRIAL_DAYS = 3   # free trial duration


class LicenseManager:
    LICENSE_FILE = "toeic_license.json"
    TRIAL_FILE   = "toeic_trial.dat"
    ...
    def __init__(self, base_dir: str):
        ...
        self._trial_start  = self._load_or_init_trial()
        self._load()
    ...
    @property
    def trial_remaining_seconds(self) -> float:
        """Seconds left in the trial. Returns inf if activated/dev, 0 if expired."""
        if self._activated or self._dev_mode:
            return float("inf")
        elapsed = time.time() - self._trial_start
        return max(0.0, TRIAL_DAYS * 86400 - elapsed)

    @property
    def can_use(self) -> bool:
        """True when either activated or still within the trial window."""
        return self.is_activated or self.trial_remaining_seconds > 0
    ...
    # ── trial persistence ─────────────────────────────────────────────────────
    def _load_or_init_trial(self) -> float:
        """Return the first-launch timestamp, creating the trial file if needed."""
        path = os.path.join(self._base_dir, self.TRIAL_FILE)
        if os.path.isfile(path):
            try:
                with open(path, encoding="utf-8") as f:
                    outer = json.load(f)
                data = json.loads(self._decode(outer.get("_", "")))
                return float(data["first_launch"])
            except Exception:
                pass   # corrupted → re-init (gives fresh 3 days)
        ts = time.time()
        payload = json.dumps({"first_launch": ts})
